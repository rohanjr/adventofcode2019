...
findOptimal prog =
  let len = length prog
      ...
      outputs = map (\phaseSeq -> runPhases phaseSeq progArray) phaseSeqs
  in maximum outputs

runPhases :: [Int] -> Array Int Int -> Int
runPhases phaseSeq prog =
  foldl' runWithPhase 0 phaseSeq
 where
  runWithPhase :: Int -> Int -> Int
  runWithPhase input phase =
    let (_, outputs) = run [phase, input] 1 prog
    in head outputs -- assume exactly one output per run

-- Return new array storage and outputs so far
run :: [Int] -> Int -> Array Int Int -> (Array Int Int, [Int])
run inputs i a =
 ...
 where
  binaryOp :: [Int] -> (Int -> Int -> Int) -> (Array Int Int, [Int])
  ...
  jump :: [Int] -> (Int -> Bool) -> (Array Int Int, [Int])
