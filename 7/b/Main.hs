...
findOptimal prog =
  let len = length prog
      ...
      outputs = map (\phaseSeq -> runLoop phaseSeq [0] 1 ips progs) phaseSeqs
  in maximum outputs

data Status = Finished | Pending Int

runLoop :: [Int] -> [Int] -> Int -> Array Int Int -> Array Int (Array Int Int) -> Int
runLoop phaseSeq inputs amp ips progs =
  let (inputs', phaseSeq') =
        case phaseSeq of
          [] -> (inputs, [])
...
run :: [Int] -> Int -> Array Int Int -> (Array Int Int, [Int], Status)
run inputs i a =
 ...
 where
  binaryOp :: [Int] -> (Int -> Int -> Int) -> (Array Int Int, [Int], Status)
  ...
  storeInput :: (Array Int Int, [Int], Status)
  storeInput =
    case inputs of
      [] -> (a, [], Pending i)
      first : rest ->
