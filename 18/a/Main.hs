...
type KeySetKeyMap = HM.HashMap Int (Map.Map Char Int)
...
distancesToEnd m@Maze{..} =
  let allDists = foldl' go HM.empty subsets
  in HM.lookup 0 allDists >>= Map.lookup '@'
 where
  subsets :: [[Int]]
  subsets =
  ...
  distFromKeyAndKeySet ds k s =
    ...
    in if missingKeys == 0 then Just 0 else
        ...
        in (\case [] -> Nothing; l -> Just $ minimum l) $
            flip mapMaybe missingKeyList $ \kk -> case Map.lookup kk pathsFromK of
              Just (MazeStateWithSteps (MazeState _ keysRequired) distToNewKey)
                | keysRequired .&. s == keysRequired ->
                    let newKeySet = setBit s (charToBitPos kk)
                        distFromNewKey = HM.lookup newKeySet ds >>= Map.lookup kk
    ...
    let ks = if s == 0 then ['@'] else intToKeys numKeys s
        insertInKeySetMap dds k =
          case distFromKeyAndKeySet ds k s of
            Nothing -> dds
            Just d -> HM.insertWith Map.union s (Map.singleton k d) dds
    in foldl' insertInKeySetMap HM.empty ks
  go :: KeySetKeyMap -> [Int] -> KeySetKeyMap
  go distances ss = HM.unions $ map (distsFromKeySet distances) ss
